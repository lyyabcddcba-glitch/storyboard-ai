...
import { guardAIRequest } from '@/app/lib/api-guard'
...
export async function POST(request: NextRequest) {
  // 认证拦截：仅 APPROVED/ADMIN 可调用
  const guardResult = await guardAIRequest(request)
  if (guardResult) return guardResult

  const { theme, style, equipment, videoType, shotCount, promptHint } = await request.json()
